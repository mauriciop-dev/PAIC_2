...
interface InternalStaffModalProps {
  isOpen: boolean;
  staffToEdit: InternalStaff | null;
  onClose: () => void;
  onSave: (staff: InternalStaff) => void;
}

const InternalStaffModal: React.FC<InternalStaffModalProps> = ({ isOpen, staffToEdit, onClose, onSave }) => {
  const [formData, setFormData] = useState<Partial<InternalStaff>>({});
  const isNew = !staffToEdit;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSave({ ...staffToEdit, ...formData } as InternalStaff);
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-2xl p-8 w-11/12 md:w-1/2 lg:w-1/3 relative" onClick={(e) => e.stopPropagation()}>
        ...
        <h2 className="text-2xl font-bold text-gray-800 mb-6">{isNew ? 'Agregar Personal Interno' : 'Editar Personal Interno'}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
           ...
           <div>
            ...
            <input type="text" id="position" name="position" value={formData.position || ''} onChange={handleChange} className="mt-1 w-full p-2 border border-gray-300 rounded-md" required />
          </div>
          <div>
            <label htmlFor="email" className="block text-sm font-medium text-gray-700">Correo Electrónico</label>
            <input type="email" id="email" name="email" value={formData.email || ''} onChange={handleChange} className="mt-1 w-full p-2 border border-gray-300 rounded-md" />
          </div>
          <div>
